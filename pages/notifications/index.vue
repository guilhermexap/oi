<template>
  <UContainer>
    <UCard>
      ...
      >
        <UInput v-model="q" placeholder="Filtrar notificação..." />
...
const openEdit = (row) => {
  setSelectedData(row);
  router.push({ name: "notifications-form" });
};
...
const updateStatus = async (status) => {
  ...
  const response = await newsServices.updateStatus(selectIds, status, userId);
  selectedRows.value = [];
  listen.value++;
  return response || [];
};
...
const { data: todos, pending } = useLazyAsyncData(
  ...
  () =>
    $fetch(
      `${apiBaseUrl}/news?nrorg=${NumberOrg}&type=N&${searchStatus.value}`,
      {
        headers: {
