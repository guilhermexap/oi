...
const openEdit = (row) => {
  router.push({
    name: "services-professional-reservation-form",
    query: {
      ID: row.ID,
      NAME: row.NAME,
      IMAGE: row.IMAGE,
      ...
      PRICE: row.PRICE,
      ICON: row.ICON,
      DSCANCELATION: row.DSCANCELATION,
...
const openNew = () => {
  router.push({ name: "services-professional-reservation-form" });
};
...
const { data: todos, pending } = useLazyAsyncData(
  ...
  () =>
    $fetch(
      `${apiBaseUrl}/services?nrorg=${NumberOrg}&${searchStatus.value}&type=AGENDA`,
      {
        headers: {
