<template>
  ...
  <UContainer>
    <UCard>
      ...
      >
        <UInput v-model="q" placeholder="Filtrar menu..." />
...
const openNew = () => {
  router.push({ name: "services-appmenu-form" });
};
...
const { data: todos, pending } = useLazyAsyncData(
  ...
  () =>
    $fetch(
      `${apiBaseUrl}/services?nrorg=${NumberOrg}&${searchStatus.value}&type=LINK,BUTTON,HTML`,
      {
        headers: {
