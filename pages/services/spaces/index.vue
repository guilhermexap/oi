  ...
  <UContainer>
    <UCard>
      ...
      <div class="flex justify-between items-center">
        <div class="col-8">
          <h5 class="font-weight-bolder mb-0" style="padding: 1rem">Espaços</h5>
        </div>
      ...
      >
        <UInput v-model="q" placeholder="Filtrar espaço..." />
...
const openEdit = (row) => {
  router.push({
    ...
    query: {
      ...
      IMAGE: row.IMAGE,
      EXTERNAL_QRCODE: row.EXTERNAL_QRCODE,
      STATUS: row.STATUS,
    },
  });
};
...
const openNew = () => {
  router.push({ name: "services-spaces-form" });
};
...
const updateStatus = async (status) => {
  ...
  const response = await spacesServices.updateStatus(selectIds, status, userId);
  selectedRows.value = [];
  listen.value++;
  return response || [];
};
...
const { data: todos, pending } = useLazyAsyncData(
  ...
  () =>
    $fetch(
      `${apiBaseUrl}/structure?nrorg=${NumberOrg}&type=L&${searchStatus.value}`,
      {
        headers: {
