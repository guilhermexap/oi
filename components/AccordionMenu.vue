  ...
  <div class="overflow-y-auto pl-6 flex-grow">
    ...
    <div v-for="item in items" :key="item.ID">
      <div class="py-1 pr-4 flex items-center justify-between">
        <div
          :class="{
            'text-primary-600 font-bold': item.ID === itemId,
          }"
          class="text-sm hover:opacity-70 transition-opacity cursor-pointer"
          @click="openEdit(item)"
        >
          <span>{{ item.ORDENATION }} {{ item.NAME }}</span>
        </div>
        <button
          v-if="item.children"
...
<script>
import { setSelectedData, getSelectedData } from "~/services/editService";
export default {
  props: {
    items: {
      type: Array,
  ...
  methods: {
    ...
    openEdit(item) {
      ...
      this.itemId = getSelectedData()?.ID || null;
      // console.log(item.ID === getSelectedData()?.ID);
